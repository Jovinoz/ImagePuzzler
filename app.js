...
class ImagePuzzler {
    ...
    renderPreview(image) {
        ...
        img.onload = () => {
            ...
            // Question text - scale with cropped image but cap to max size and screen width
            const questionEl = document.createElement('div');
            questionEl.className = 'preview-question';
            questionEl.textContent = image.question || '';
            questionEl.style.whiteSpace = 'nowrap';
            // Start with scaled size, but cap at the user-specified max size
            let questionFontSize = Math.min(image.questionSize * croppedScale, image.questionSize);
            questionEl.style.fontSize = questionFontSize + 'px';
            questionEl.style.opacity = '0';

            // Measure text width and reduce font if needed to fit screen
            if (image.question) {
                // Temporarily add to DOM to measure (positioned off-screen)
                questionEl.style.position = 'absolute';
                questionEl.style.left = '-9999px';
                document.body.appendChild(questionEl);
                const maxQuestionWidth = window.innerWidth * 0.95;
                while (questionEl.offsetWidth > maxQuestionWidth && questionFontSize > 10) {
                    questionFontSize *= 0.9;
                    questionEl.style.fontSize = questionFontSize + 'px';
                }
                document.body.removeChild(questionEl);
                questionEl.style.position = '';
                questionEl.style.left = '';
            }
    ...
    <script>
        ...
        function loadQuestion() {
            ...
            img.onload = () => {
                ...
                // Set question - scale with cropped image but cap to max size and screen width
                questionText.textContent = q.question;
                // Start with scaled size, but cap at the user-specified max size
                let questionFontSize = Math.min(q.questionSize * croppedDisplayScale, q.questionSize);
                questionText.style.fontSize = questionFontSize + 'px';
                // Reduce font size if text exceeds viewport width
                const maxTextWidth = window.innerWidth * 0.95;
                while (questionText.offsetWidth > maxTextWidth && questionFontSize > 10) {
                    questionFontSize *= 0.9;
                    questionText.style.fontSize = questionFontSize + 'px';
                }
